...
private:
    ...
    Array<pair> A[size];
public:
    Mass_Pair(); // êîíñòðóêòîð
    Mass_Pair(int ); // êîíñòðóêòîð ñ ïàðàìåòðàìè
    Mass_Pair(const Mass_Pair & ); // êîíñòðóêòîð êîïèé
    ~Mass_Pair(); // äåñòðóêòîð

    int getSize(); // âîçâðàòèòü ðàçìåð
    void set_Mass_Pair();
    void get_Mass_Pair();
    void push_back(pair x);
    void push_front(pair x);
    void turn(int k);
       /*òóò îïèñàíû òîëüêî push_back/front è ïðîñòåéøèå îïåðàòîðû, âñå
       îñòàëüûíå ôóíêöèè (ñîðòèðîâêè, ïîèñê - áèíàðíûé è ëèíåéíûé) òî÷íî òàêèå æå, êàê â
       ïðåäûäóùåé ðàáîòå (class1) òîëüêî ñ èçìåíåíèåì òèïà äàííûõ íà ïàðû*/
    pair operator[](int a);
    const Mass_Pair& operator+(const Mass_Pair& A);
    const Mass_Pair& operator=(const Mass_Pair& A);
};
Mass_Pair::Mass_Pair() // êîíñòðóêòîð ïî óìîë÷àíèþ, áåç ïàðàìåòðîâ
{
    size=10;
    Arr=new pair [size];
    ...
    :size(arrayToCopy.size)
{
    ...
    Arr.first[1]=arrayToCopy.Arr.first[size-1]; /*öèêëè÷íîñòü*/
    Arr.second[1]=arrayToCopy.Arr.second[size-1];
}

Mass_Pair::~Mass_Pair() // äåññòðóêòîð êëàññà
{
    delete  []A;
}
const Mass_Pair& operator=(const Mass_Pair& A)
